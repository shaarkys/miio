...
const SOFT_NETWORK_ERROR_CODES = new Set([
  "ENETUNREACH",
  "EHOSTUNREACH",
  "ETIMEDOUT",
  "ECONNREFUSED",
...
function isSoftNetworkError(err) {
  return Boolean(err && SOFT_NETWORK_ERROR_CODES.has(err.code));
}
...
class DeviceInfo {
  ...
  async enrich() {
    ...
    if (this.packet.token) {
      ...
      this.autoToken = false;
      try {
        const storedToken = await tokens.get(this.id);
        if (storedToken) {
          this.debug("Using stored token:", storedToken);
          this.packet.token = Buffer.from(storedToken, "hex");
          this.tokenChanged = true;
        }
      } catch (err) {
        this.debug("Could not load stored token", err);
      }
    }

    // Check if a model was already provided, e.g. through discovery
    if (this.model) {
      this.enriched = true;
      this.tokenChanged = false;
      return Promise.resolve();
    ...
    return await new Promise((resolve, reject) => {
      ...
      this.parent.socket.send(data, 0, data.length, this.port, this.address, (err) => {
        ...
        if (isSoftNetworkError(err)) {
          // Let the handshake/call timeout drive retry logic for transient network failures
          // instead of failing fast.
          this.debug("Packet send soft network error", err);
          return;
        }
    ...
    return await this._retryOnTimeout(retries, async (retriesLeft) => {
      ...
      try {
        ...
        if (!(err instanceof Error) && typeof err.code !== "undefined") {
          ...
          const msg = handler ? handler(method, params, err) : err.message || err.toString();
  ...
  async _retryOnTimeout(retries = 5, actionPromiseFn) {
    while (retries > 0) {
      try {
        const result = await Promise.race([actionPromiseFn(retries), this._setTimeout()]);
        return result;
      } catch (err) {
        if (err.code !== "timeout") {
          throw err;
