...
const { TimedDiscovery, ServiceDiscovery, search } = require("tinkerhub-discovery");
...
const Browser = (module.exports.Browser = class Browser extends TimedDiscovery {
  ...
  [tryAdd](device) {
    if (!device.id) {
      console.warn("Skipping device with missing ID:", device.address);
      return;
    }
    try {
      const service = {
...
class Devices extends ServiceDiscovery {
  static get type() {
    return "miio:devices";
  }
  ...
  _bindSubDevices(device) {
    try {
      ...
      device.on("thing:unavailable", (sub) => this.remove(sub.id));
